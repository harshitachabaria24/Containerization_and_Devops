# 🚀 Containerization & DevOps Lab Experiments

👩‍💻 **Name:** Harshita Chabaria
🎓 **Course:** B.Tech CSE (Cloud Computing)
📚 **Subject:** Containerization and DevOps

---

## 📌 About This Repository

This repository contains a complete set of **DevOps and Containerization lab experiments** covering:

* Docker & Docker Compose
* CI/CD using Jenkins
* Infrastructure Automation using Ansible
* Code Quality using SonarQube
* Container Orchestration using Docker Swarm & Kubernetes

Each experiment includes:
✔️ Theory
✔️ Commands
✔️ Configuration files
✔️ Outputs
✔️ Screenshots

---

## 🧪 List of Experiments

| Experiment No. | Title                                      |
| -------------- | ------------------------------------------ |
| [Lab 1](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab1.md)          | Virtual Machines vs Containers (Nginx Setup using VirtualBox/Vagrant & Docker)
| [Lab 2](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab2.md)          | Docker Basics: Images, Containers & Lifecycle Management
| [Lab 3](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab3.md)          | NGINX Deployment using Docker (Ubuntu vs Alpine vs Official Image)
| [Lab 4](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab4.md)          | Docker Essentials           
| [Lab 5](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab5.md)          | Docker Advanced Concepts: Volumes, Environment Variables, Monitoring & Networks                           
| [Lab 6](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab6.md)          | Docker Run vs Docker Compose               |
| [Lab 7](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab7.md)          | CI/CD using Jenkins, GitHub & Docker Hub   |
| [Lab 9](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab9.md)          | Configuration Management using Ansible     |
| [Lab 10](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab10.md)         | Code Quality Analysis using SonarQube      |
| [Lab 11](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab11.md)        | Container Orchestration using Docker Swarm |
| [Lab 12](https://github.com/harshitachabaria24/Containerization_and_Devops/blob/main/LAB/lab12.md)         | Container Orchestration using Kubernetes   |

---

## 🛠️ Technologies Used

* 🐳 Docker
* 🧩 Docker Compose
* ⚙️ Jenkins
* 🔁 Git & GitHub
* 📦 Docker Hub
* 🤖 Ansible
* 🔍 SonarQube
* ☸️ Kubernetes (Minikube)
* 🐧 Linux

---

## 📂 Repository Structure

```bash
.
├── Lab-1/
├── Lab-2/
├── Lab-3/
├── Lab-4/
├── Lab-5/
├── Lab-6/
├── Lab-7/
├── Lab-9/
├── Lab-10/
├── Lab-11/
├── Lab-12/
└── README.md
```

---

## 🎯 Key Learnings

* ✔️ Containerization using Docker
* ✔️ Multi-container apps using Docker Compose
* ✔️ CI/CD pipeline automation
* ✔️ Infrastructure as Code (Ansible)
* ✔️ Code quality & static analysis
* ✔️ Container orchestration (Swarm + Kubernetes)
* ✔️ Scaling & self-healing systems

---

## 🔥 Highlights

* 🚀 Built complete CI/CD pipeline (GitHub → Jenkins → Docker Hub)
* 📦 Deployed multi-container applications
* ⚡ Automated infrastructure using Ansible
* 🔍 Performed code quality analysis using SonarQube
* ☸️ Implemented Kubernetes scaling & self-healing
* 🐳 Orchestrated services using Docker Swarm
---

## 🧠 Conclusion

This repository demonstrates practical implementation of **modern DevOps practices**, including automation, containerization, CI/CD, and orchestration — essential skills for real-world software deployment and scalability.

---

## ⭐ Future Improvements

* Add Kubernetes Helm charts
* Implement monitoring (Prometheus + Grafana)
* Add GitHub Actions pipeline

---

## 🙌 Acknowledgement

This lab work is part of academic coursework and reflects hands-on learning in DevOps tools and practices.

---

